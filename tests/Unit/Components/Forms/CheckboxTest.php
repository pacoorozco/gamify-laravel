...
final class CheckboxTest extends TestCase
{
    ...
    public function it_should_correctly_render_with_defaults(): void
    {
        $view = $this->withViewErrors([])
            ->blade(
            '<x-forms.checkbox :name="$name" :value="$value" :label="$label"/>',
    ...
    public function it_should_render_custom_id_and_for(): void
    {
        $view = $this->withViewErrors([])
            ->blade(
                '<x-forms.checkbox :name="$name" :value="$value" :label="$label" id="myUniqueId"/>',
                ['name' => 'test', 'value' => 'my-value', 'label' => 'The Input Label']
            );
    ...
    public function it_should_check_checkbox(): void
    {
        $view = $this->withViewErrors([])
            ->blade(
                '<x-forms.checkbox :name="$name" :value="$value" :label="$label" :checked="true"/>',
                ['name' => 'test', 'value' => 'my-value', 'label' => 'The Input Label']
            );
    ...
    public function it_should_disable_checkbox(): void
    {
        $view = $this->withViewErrors([])
            ->blade(
                '<x-forms.checkbox :name="$name" :value="$value" :label="$label" :disabled="true"/>',
                ['name' => 'test', 'value' => 'my-value', 'label' => 'The Input Label']
            );
    ...
    public function it_should_render_additional_classes(): void
    {
        $view = $this->withViewErrors([])
            ->blade(
                '<x-forms.checkbox :name="$name" :value="$value" :label="$label" :class="$class"/>',
                ['name' => 'test', 'value' => 'my-value', 'label' => 'The Input Label', 'class' => 'is-large is-rounded is-static']
            );

        $view->assertSee('class="checkbox is-large is-rounded is-static"', false);
    }
}
